...
const PUBLIC_PAGES = ['Home', 'About', 'Media', 'Ministries', 'Events', 'Giving', 'Contact'];

const NAV_GROUPS = [
  { label: 'Home', page: 'Home' },
  { label: 'About', page: 'About', items: [
  ...
  { label: 'Ministries', page: 'Ministries', items: [
    { label: 'Discipleship School', page: 'Discipleship' },
...
function getInitialPage() {
  ...
  const pages = ['Home','About','Media','Ministries','Programs','Sermons','Discipleship','Events','Testimonies','Prayer','Giving','Contact'];
  return pages.find((p) => p.toLowerCase() === slug) || 'Home';
}

function runSelfTests() {
  return [
    { name: 'Public navigation is reduced', passed: PUBLIC_PAGES.length === 7 },
    { name: 'Admin is hidden from public menu', passed: !PUBLIC_PAGES.includes('Admin') },
    { name: 'Dropdown menus exist', passed: NAV_GROUPS.some((g) => g.items?.length) },
    { name: 'Homepage video file path is configured', passed: true },
    { name: 'Events loaded', passed: eventsSeed.length >= 3 }
  ];
}
...
function App() {
  ...
  const filteredSermons = useMemo(() => {
    ...
    return sermons.filter((s) => {
      ...
      const searchMatch = !term || [s.title, s.preacher, s.scripture, s.category].join(' ').toLowerCase().includes(term);
      return categoryMatch && searchMatch;
    });
  }, [sermons, search, category]);

  function goToPage(nextPage) {
    window.history.pushState({}, '', nextPage === 'Home' ? '/' : `/${nextPage.toLowerCase()}`);
    setPage(nextPage);
    setMenuOpen(false);
  ...
  const props = {
    ...
    adminOk, setAdminOk, tests, submitPrayer, submitTestimony, addSermon, addEvent, approveTestimony, setNotice
  };
...
function Header({ active, goToPage, menuOpen, setMenuOpen }) {
  function groupActive(group) {
    return active === group.page || Boolean(group.items?.some((item) => item.page === active));
  }
    ...
    <header className="topbar">
      ...
      <div className="navwrap">
        ...
        <nav className="desktopNav">
          {NAV_GROUPS.map((group) => (
            <div className="navGroup" key={group.label}>
              <button className={groupActive(group) ? 'active' : ''} onClick={() => goToPage(group.page)}>
        ...
        <div className="navCtas">
          <button className="outlineBtn" onClick={() => goToPage('Prayer')}>Request Prayer</button>
          <button className="goldBtn small" onClick={() => goToPage('Giving')}>Give</button>
        </div>
        ...
        <div className="mobileNav">
          {NAV_GROUPS.map((group) => (
            <div key={group.label} className="mobileGroup">
              <button className={groupActive(group) ? 'active' : ''} onClick={() => goToPage(group.page)}>{group.label}</button>
              {group.items && <div className="mobileSubnav">{group.items.map((item) => <button key={item.label} onClick={() => goToPage(item.page)}>{item.label}</button>)}</div>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <>
      ...
      <section className="prayerBand">
        <div><p className="eyebrow light">How can we pray for you?</p><h2>Whatever you are concerned about, we are here to pray with you.</h2></div>
      ...
      <section className="donateBand">
        <h2>Give a donation and help share the hope of Christ.</h2>
        <p>Support tithes, offerings, missions, outreach, discipleship school and ministry work locally and internationally.</p>
        <button className="goldBtn" onClick={() => goToPage('Giving')}>Give Online</button>
      </section>
    </>
  );
}
...
function AboutPage() {
  return <PageShell eyebrow="About Us" title="Who we are"><div className="cardGrid"><InfoCard title="Vision" text="To see lives healed, restored, discipled, and empowered through the living Word of God." /><InfoCard title="Mission" text="To preach Christ, teach the Word, raise disciples, support families, and reach communities and nations." /><InfoCard title="Leadership" text="Pastor profile, leadership team, statement of faith and ministry values will be published here." /></div></PageShell>;
}

function MediaPage({ goToPage, sermons, setNotice }) {
  return <PageShell eyebrow="Media" title="Sermons, audio messages, live stream and YouTube teaching"><div className="twoCol"><div className="darkCard"><h2>Watch and listen to the Word</h2><p>Access sermon videos, audio messages, worship moments, live stream links and teachings from Send His Word Ministries.</p><button className="goldBtn" onClick={() => goToPage('Sermons')}>Open Sermons Library</button></div><div className="whiteCard"><h3>YouTube Channel</h3><p>Connect this area to the official Send His Word Ministries YouTube channel for preaching, worship and live services.</p><a className="darkBtn linkBtn" href="https://www.youtube.com/@sendhiswordministries9520" target="_blank" rel="noreferrer">Visit YouTube Channel</a></div></div><div className="cardGrid topSpace">{sermons.slice(0, 3).map((s) => <SermonCard key={s.id} sermon={s} setNotice={setNotice} />)}</div></PageShell>;
}

function MinistriesPage({ goToPage }) {
  return <PageShell eyebrow="Ministries" title="Discipleship, weekly programs, outreach and worship ministry"><div className="cardGrid"><InfoCard title="Discipleship School" text="A structured school for believers who want to grow deeper in the Word, prayer, character and service." /><InfoCard title="Weekly Programs" text="Sunday celebration service, midweek Bible study, youth fellowship, prayer and worship ministry." /><InfoCard title="Outreach Ministry" text="Community outreach, evangelism, missions support and practical ministry to families." /></div><div className="donateBand"><h2>Grow, serve and become part of what God is doing.</h2><p>Explore discipleship school and weekly programs.</p><button className="goldBtn" onClick={() => goToPage('Discipleship')}>Join Discipleship School</button></div></PageShell>;
}

function ProgramsPage() {
  return <PageShell eyebrow="Programs" title="Daily, weekly, and monthly ministry programs"><div className="cardGrid"><Program title="Daily" items={['Morning devotion', 'Prayer altar', 'Counselling support', 'WhatsApp encouragement']} /><Program title="Weekly" items={['Sunday celebration service', 'Midweek Bible study', 'Youth fellowship', 'Choir and worship practice']} /><Program title="Monthly" items={['Healing service', 'Leaders training', 'Outreach ministry', 'Conferences and special meetings']} /></div></PageShell>;
}

function SermonsPage({ filteredSermons, categories, search, setSearch, category, setCategory, setNotice }) {
  return <PageShell eyebrow="Sermons" title="Watch, listen, search, and grow through the Word"><div className="toolbar"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by sermon, preacher, scripture..." /><div className="pillRow">{categories.map((c) => <button key={c} className={category === c ? 'pill active' : 'pill'} onClick={() => setCategory(c)}>{c}</button>)}</div></div><div className="cardGrid">{filteredSermons.map((s) => <SermonCard key={s.id} sermon={s} setNotice={setNotice} />)}</div>{!filteredSermons.length && <div className="empty">No sermons match your search yet.</div>}</PageShell>;
}

function DiscipleshipPage({ setNotice }) {
  return <PageShell eyebrow="Discipleship School" title="A structured school for believers who want to grow deeper"><div className="twoCol"><div className="darkCard"><h2>Enroll in Discipleship School</h2><p>Designed for new believers, church workers, leaders, and anyone hungry to grow in the Word, prayer, character, service, and ministry.</p><p><b>Duration:</b> 3–6 months</p><p><b>Mode:</b> Physical and online learning support</p></div><form className="whiteCard form"><h3>Student Registration</h3><Input label="Full Name" /><Input label="Phone Number" /><Input label="Email Address" /><Input label="Preferred Class" /><textarea placeholder="Tell us why you want to join..." /><button type="button" className="darkBtn" onClick={() => setNotice('Discipleship registration sample submitted.')}>Submit Registration</button></form></div></PageShell>;
}
...
function TestimoniesPage({ testimonies, submitTestimony }) {
  return <PageShell eyebrow="Testimonies" title="Stories of healing, restoration, salvation, and answered prayer"><div className="twoCol"><div className="stack">{testimonies.filter((t) => t.approved).map((t) => <TestimonyCard key={t.id} testimony={t} />)}</div><form onSubmit={submitTestimony} className="darkCard form"><h3>Share Your Testimony</h3><Input dark label="Your Name" name="name" /><Input dark label="Testimony Title" name="title" /><select name="type"><option>Written</option><option>Video</option></select><textarea name="text" required placeholder="Write your testimony..." /><button className="goldBtn">Submit Testimony</button></form></div></PageShell>;
}

function PrayerPage({ submitPrayer }) {
  return <PageShell eyebrow="Prayer Requests" title="We believe in prayer. Submit your request and our team will stand with you"><div className="twoCol"><div className="goldCard"><h2>Need Prayer?</h2><p>Share your prayer need for healing, family, finances, salvation, marriage, business, or spiritual growth.</p><a className="darkBtn linkBtn" href="https://wa.me/256700000000" target="_blank" rel="noreferrer">Chat on WhatsApp</a></div><form onSubmit={submitPrayer} className="whiteCard form"><Input label="Full Name" name="name" /><select name="category"><option>Healing</option><option>Family</option><option>Finances</option><option>Salvation</option><option>Marriage</option><option>Business</option><option>Spiritual Growth</option></select><select name="privacy"><option>Private request</option><option>Public prayer wall</option></select><textarea name="message" required placeholder="Write your prayer request..." /><button className="darkBtn">Submit Prayer Request</button></form></div></PageShell>;
}
...
function GivingPage({ givingType, setGivingType }) {
  ...
  return <PageShell eyebrow="Giving" title="Partner with Send His Word Ministries locally and internationally"><div className="twoCol"><div className="whiteCard"><h3>Choose Giving Purpose</h3><div className="givingGrid">{types.map((t) => <button key={t} className={givingType === t ? 'giving active' : 'giving'} onClick={() => setGivingType(t)}>{t}</button>)}</div><p>Selected purpose: <b>{givingType}</b>. Connect Flutterwave, PayPal, Stripe, MTN Mobile Money, Airtel Money, and international bank transfer details.</p></div><div className="cardGrid two"><InfoCard title="MTN Mobile Money" text="Local giving through MTN MoMo collections." /><InfoCard title="Airtel Money" text="Local giving through Airtel Money collections." /><InfoCard title="Card Payments" text="Visa and Mastercard for international givers." /><InfoCard title="International Giving" text="PayPal, Stripe, SWIFT, Remitly and WorldRemit guidance." /></div></div></PageShell>;
}

function ContactPage() {
  return <PageShell eyebrow="Contact" title="Visit, call, email, or chat with us on WhatsApp"><div className="twoCol"><div className="darkCard"><h2>Send His Word Ministries</h2><p>☎ +256 700 000 000</p><p>✉ info@sendhiswordministries.org</p><p>📍 Kampala, Uganda</p><p>💬 WhatsApp prayer and inquiry line</p><a className="goldBtn linkBtn" href="https://wa.me/256700000000" target="_blank" rel="noreferrer">Chat With Us on WhatsApp</a></div><div className="mapBox">📍<br />Google Map Embed Area<br /><small>Replace this with the official church Google Maps embed code.</small></div></div></PageShell>;
}

function AdminPage({ adminOk, setAdminOk, adminTab, setAdminTab, sermons, events, prayerRequests, testimonies, addSermon, addEvent, approveTestimony, tests }) {
  ...
  return <PageShell eyebrow="Admin" title="Content management dashboard"><div className="adminBox"><div className="pillRow">{tabs.map((t) => <button key={t} className={adminTab === t ? 'pill active' : 'pill'} onClick={() => setAdminTab(t)}>{t}</button>)}</div>{adminTab === 'Sermons' && <AdminPanel title="Sermon Management" button="Add Sermon" onClick={addSermon} items={sermons.map((s) => `${s.title} — ${s.category}`)} />}{adminTab === 'Events' && <AdminPanel title="Event Management" button="Add Event" onClick={addEvent} items={events.map((e) => `${e.title} — ${e.date}`)} />}{adminTab === 'Prayer' && <AdminPanel title="Prayer Inbox" button="Refresh" onClick={() => {}} items={prayerRequests.length ? prayerRequests.map((r) => `${r.name} — ${r.category} — ${r.privacy}`) : ['No prayer requests submitted yet.']} />}{adminTab === 'Testimonies' && <div className="adminPanel"><h3>Approve Testimonies</h3>{testimonies.map((t) => <div className="adminItem" key={t.id}><span>{t.title} — {t.name} — {t.approved ? 'Published' : 'Pending'}</span>{!t.approved && <button onClick={() => approveTestimony(t.id)}>Approve</button>}</div>)}</div>}{adminTab === 'Tests' && <div className="adminPanel"><h3>Smoke Tests</h3>{tests.map((t) => <div className="adminItem" key={t.name}><span>{t.name}</span><b>{t.passed ? 'PASS' : 'FAIL'}</b></div>)}</div>}</div></PageShell>;
}
